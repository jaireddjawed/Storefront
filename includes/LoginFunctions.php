<?php
  ...
  function checkUserRole($connection)
  {
    $isLoggedIn = isset($_SESSION['user_id'], $_SESSION['email_address'], $_SESSION['login_string']);

    if ($isLoggedIn)
    {
      $userId = $_SESSION['user_id'];

      $roleStmt = 'SELECT `role` FROM `users` WHERE `id` = ? LIMIT 1';
      $roleStmt = $connection->prepare($roleStmt);

      if (!$roleStmt) {
        echo('Could not get the user role.');
        exit;
      }
  ...
  {
    $userStmt = 'SELECT `id`, `first_name`, `last_name`, `password`, `salt` FROM `users` WHERE `email_address` = ? LIMIT 1';
    $userStmt = $connection->prepare($userStmt);
    ...
    $userStmt->bind_result($userId, $firstName, $lastName, $dbPassword, $salt);
    $userStmt->fetch();
